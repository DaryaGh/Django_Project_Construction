...
def news_comment(request, id):
    if request.method == 'POST':
        try:
            ...
            new_comment = Comment(
                username_comment=request.user.username if request.user.is_authenticated else data.get('username'),
                body=data.get('comment'),
                email_comment=data.get('email'),
                website_comment=data.get('website'),
                object_id=id,
                content_type=ContentType.objects.get_for_model(News),
                parent_id=parent_id  # مستقیماً از parent_id استفاده کنید
            )
            new_comment.save()

            return JsonResponse({'success': 'Comment added successfully'}, status=200)
